...
\begin{itemize}
  \item Extract the moving object's silhouette from each video frame (green-screen segmentation).
...
task3/video.mp4
\end{lstlisting}
The video contains a dark silhouette on a bright green background, which makes segmentation robust.
...
We segment the silhouette using HSV thresholding for the green background:
\begin{itemize}
  \item mask\_green = \texttt{inRange(HSV, green\_low, green\_high)}
  \item silhouette = \texttt{NOT(mask\_green)}
  \item Morphological open/close removes small artifacts.
\end{itemize}
We also ignore a small bottom strip (\texttt{--ignore-bottom-frac}) to avoid spurious contours.
...
\begin{itemize}
  \item Contour tracking assumes a clean silhouette; complex backgrounds would require a stronger segmentation model.
